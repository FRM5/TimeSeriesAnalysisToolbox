...
for (i in 1:length(dur)){
  ...
  gev <- fevd(val, maxd, type="GEV")
  gmb <- fevd(val, maxd, type="Gumbel")
  fitgev <- summary(gev)
  fitgmb <- summary(gmb)
  
  #Choosing the best fit using AIC and BIC
  if (fitgev$AIC & fitgev$BIC < fitgmb$AIC & fitgmb$BIC){
    fit <- gev}
  else {fit <- gmb}
  select <- return.level(fit, conf = 0.05, return.period = RP)
  output[i] <- as.numeric(select)
} 
