...
#load data
raindata = read.table("2005.txt", header = TRUE)

# convert to date and time and add 0.1 mm precipitation
dates <-strptime(paste(raindata$date, raindata$time), "%m/%d/%Y %H:%M:%S", tz = "UTC")
raindata$date <- dates
raindata["precip"] <-  0.1
raindata$time <- NULL
...
data_30min <- na.omit(rollapply(min_ts, 30, sum, align = 'left'))/0.5
min_30 <- tapply(data_30min$data1m, year(index(data_30min)), max)
rm(data_30min)
data_1h <- na.omit(rollapply(min_ts, 60, sum, align = 'left'))
hour_1 <- na.omit(tapply(data_1h$data1m, year(index(data_1h)), max))
rm(data_1h)
...
# create return periods and calculate rain intensity for each duration and return period
...
# ploting results
plot(gvisLineChart(all_data, xvar = "duration", yvar = c("5 year", "10 year", "25 year", "50 year", "100 year", "200 year"), options = list(gvis.editor = "Edit chart", title = "IDF curve for February 1994 to August 2016 rainfall data", vAxes="[{title:'intensity (mm/hour)'}]", hAxes="[{title:'duration (hour)'}]", curveType = "function", width = 1300, height = 700)))