...
# Create IDF curves from tipping bucket precipitation data
...
setwd("~/Documents/UPC/Droughts/Assignment")    
...
#  if (i==1){
#    Year_Max <- aggregate(Mov_Avg$Precipitation ~ year(Mov_Avg$date), FUN = max)
#  }else{
#    Mov_Avg <- aggregate(Mov_Avg$Precipitation ~ year(Mov_Avg$date), FUN = max)
#    Year_Max<-merge(Year_Max, Mov_Avg, by="year(Mov_Avg$date)")
#  }
#  if(Duration[i]<60){
#    Colname=paste0(Duration[i], "min")
#  }else{
#    Colname=paste0(Duration[i]/60, "hr")
#  }
#  Year_Max<-rename(Year_Max, replace = c(TS_Agg = Colname))
#}
#Year_Max<-rename(Year_Max, replace = c("year(Mov_Avg$date)" = "Year"))
...
AIC_GEV <- summary(Fit_GEV, silent=TRUE)
AIC_GEV <- c(AIC_GEV$AIC)
AIC_Gum <- summary(Fit_Gum, silent=TRUE)
...
Col_Names_RP <- matrix(data=NA)
for (i in 1:length(Return_Period)){
  Col_Names_RP[i] <- paste0(Return_Period[i], " years")
}
colnames(IDF_Table) <- c(Col_Names_RP)
Hours <- as.data.frame(c(as.numeric(as.character(t(Duration))))/60) 
colnames(Hours) <-"Duration [hrs]"
IDF_Table <- cbind(Hours, IDF_Table)
IDF_Plot<- melt(IDF_Table, id.vars="Duration [hrs]")
