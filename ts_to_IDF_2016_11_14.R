...
library(QRM) # for gumbel and goodness of fit
...
FrequencyAnalysis <- dget("FrequencyAnalysis.R")

# Note - Before running this script, you need to:
#        set the working directory to the proper folder
#        remove the header of your input data before running this script
#        verify the year/duration/return periods in the following section
#          if these are not suitable, additional changes will be required throughout the script

############################ CREATE SUPPORTING OBJECTS

# Need to edit this if your data has different years (should make this flow from the read-in process)
yrlist <- c(2005:2016)
n = length(yrlist)
...
# Note - in this dataset, each entry (each "tick") represents 0.1 mm of water (like a tipping bucket)
mydata = read.table("15nh.txt")
colnames(mydata)[1] = "Date"
colnames(mydata)[2] = "Time"

# Convert mydata to time series: concatenate date and time, then stores as a POSIXlt POSIXt object
#   Note - stored in central european summer time (timezone shouldn't matter for this analysis)
...
# Flip annomax in case someone wants to make a visual comparison to observed data
...
# Next step: find a way to alternate the incremental series to make a hyetograph
